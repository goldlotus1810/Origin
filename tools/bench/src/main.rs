...
fn main() {
    ...
    println!(
        "Processed: {} texts in {:.1}ms",
        sample_size,
        rt_elapsed.as_millis()
    );
    ...
    for (text, label) in rows.iter().take(10) {
        let records = book.read(text);
        let avg_v = if records.is_empty() {
            0.0
        } else {
            records.iter().map(|r| r.emotion.valence).sum::<f32>() / records.len() as f32
        };
        let predicted = if avg_v > 0.15 {
            "pos"
        } else if avg_v < -0.15 {
    ...
    // 3a. UCD Lookup
    {
        ...
        println!(
            "UCD lookup   : {:.0} ops/s ({:.1}µs per batch of {})",
            ops,
    ...
    // 3c. Silk co_activate
    {
        ...
        let iters = 10000;
        for i in 0..iters {
            graph.co_activate(i as u64, (i + 1) as u64, EmotionTag::NEUTRAL, 0.5, i as i64);
        }
        let elapsed = t.elapsed();
        let ops = iters as f64 / elapsed.as_secs_f64();
        println!(
            "Silk activate: {:.0} ops/s ({:.1}µs per call)",
            ops,
            elapsed.as_micros() as f64 / iters as f64
        );
    }
    ...
    // 3d. FFR point generation
    {
        ...
        let ops = iters as f64 / elapsed.as_secs_f64();
        println!(
            "FFR point    : {:.0} ops/s ({:.1}µs per 1000)",
            ops,
            elapsed.as_micros() as f64 / (iters as f64 / 1000.0)
        );
    }
    ...
    // 3e. chain_hash
    {
        ...
        let ops = iters as f64 / elapsed.as_secs_f64();
        println!(
            "chain_hash   : {:.0} ops/s ({:.1}ns per call)",
            ops,
