...
fn main() {
    let path = env::args().nth(1).unwrap_or_else(|| "origin.olang".to_string());
    ...
    if !parsed.nodes.is_empty() {
        ...
        for node in parsed.nodes.iter().take(10) {
            ...
            println!("  [{qr}] L{} hash=0x{:08X} mols={} ts={}",
                node.layer, hash & 0xFFFFFFFF, mol_count, node.timestamp);
        }
    ...
    if !parsed.aliases.is_empty() {
        ...
        for alias in &aliases {
            println!("  {:?} -> 0x{:08X}", alias.name, alias.chain_hash & 0xFFFFFFFF);
        }
    }
    ...
    let empty_chains = parsed.nodes.iter()
        .filter(|n| n.chain.is_empty())
        .count();
    if empty_chains == 0 {
    ...
    let offsets_ok = parsed.nodes.windows(2)
