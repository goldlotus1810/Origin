...
impl FitResult {
    ...
    pub fn is_ignorable(&self) -> bool {
        self.confidence < IGNORE_THRESHOLD
    }
}
...
impl SdfFitter {
    ...
    pub fn fit_best(&self, points: &[Vec3]) -> Option<FitResult> {
        ...
        // Thử từng primitive
        let candidates = [
            self.try_fit(SdfKind::Sphere, points, center, SdfParams::sphere(r)),
            self.try_fit(
                SdfKind::Box,
                points,
                center,
            ...
            self.try_fit(SdfKind::Cone, points, center, SdfParams::cone(r, extents.y)),
            self.try_fit(
                SdfKind::Cylinder,
                points,
                center,
            ...
            self.try_fit(SdfKind::Octahedron, points, center, SdfParams::sphere(r)),
        ];

        candidates.into_iter().flatten().max_by(|a, b| {
            a.confidence
...
mod tests {
    ...
    fn fit_sphere_high_confidence() {
        ...
        assert!(
            ...
            "Sphere cloud → Sphere/Ellipsoid, got {:?}",
            result.kind
        );
        assert!(
            result.confidence >= ACCEPT_THRESHOLD,
            "confidence={:.3} ≥ {}",
            result.confidence,
            ACCEPT_THRESHOLD
        );
    ...
    fn fit_box_reasonable() {
        ...
        let r = result.unwrap();
        assert!(
            r.confidence >= IGNORE_THRESHOLD,
            "Box fit confidence={:.3} ≥ {}",
            r.confidence,
            IGNORE_THRESHOLD
        );
    }
    ...
    fn kind_to_codepoint_all_valid() {
        ...
        for b in 0x01u8..=0x12 {
            ...
            assert!(
                cp > 0x20,
                "Codepoint phải là printable Unicode: 0x{:04X}",
                cp
            );
        }
    }
