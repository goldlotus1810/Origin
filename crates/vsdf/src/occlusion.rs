...
impl OcclusionBuffer {
    ...
    pub fn latest(&self) -> Option<&OcclusionFrame> {
        ...
        let idx = if self.head == 0 { DEFAULT_BUFFER_SIZE - 1 } else { self.head - 1 };
    ...
    pub fn is_occluded(&self) -> bool {
        ...
        self.frames.iter().all(|f| f.as_ref().is_some_and(|fr| fr.distance < 0.0))
    }
    ...
    pub fn movement_variance(&self) -> f32 {
        ...
        let var = frames.iter().map(|f| (f.distance - mean) * (f.distance - mean)).sum::<f32>()
            / frames.len() as f32;
        var
    }
}
...
mod tests {
    ...
    fn push_and_latest() {
        ...
        buf.push(OcclusionFrame { distance: 1.0, shape_idx: 0, timestamp: 100 });
        assert_eq!(buf.len(), 1);
    ...
    fn ring_buffer_wraps() {
        let mut buf = OcclusionBuffer::new();
        for i in 0..8 {
            buf.push(OcclusionFrame { distance: i as f32, shape_idx: 0, timestamp: i * 100 });
        }
    ...
    fn occluded_when_all_negative() {
        ...
        for i in 0..DEFAULT_BUFFER_SIZE {
            buf.push(OcclusionFrame { distance: -0.5, shape_idx: 0, timestamp: i as i64 });
        }
        assert!(buf.is_occluded());
    }
    ...
    fn not_occluded_when_mixed() {
        ...
        for i in 0..DEFAULT_BUFFER_SIZE {
            ...
            buf.push(OcclusionFrame { distance: d, shape_idx: 0, timestamp: i as i64 });
        }
        assert!(!buf.is_occluded());
    }
    ...
    fn movement_variance_detects_motion() {
        ...
        for i in 0..5 {
            buf.push(OcclusionFrame { distance: 1.0, shape_idx: 0, timestamp: i });
        }
        ...
        for i in 0..5 {
            buf2.push(OcclusionFrame { distance: i as f32 * 2.0, shape_idx: 0, timestamp: i });
        }
