...
impl VectorField {
    ...
    pub fn sunlight() -> Self {
        ...
        // Bình minh lên
        intensity.push(BezierSegment { p0: 0.05, p1: 0.05, p2: 0.95, p3: 0.95 });
        // Giữa trưa xuống
        intensity.push(BezierSegment { p0: 0.95, p1: 0.95, p2: 0.05, p3: 0.05 });
...
mod tests {
    ...
    fn sunlight_ambient_floor() {
        ...
        for t in [0.0f32, 0.05, 0.95, 1.0] {
            ...
            assert!(i >= sun.ambient, "Ambient floor tại t={}: {} >= {}", t, i, sun.ambient);
        }
    }
    ...
    fn emotion_neutral() {
        ...
        assert!((s.arousal - 0.5).abs() < 0.01, "Neutral A=0.5: {}", s.arousal);
    }
    ...
    fn emotion_arc_shape() {
        ...
        // Đau nhất ở peak
        assert!(peak.valence < start.valence,
            "Peak buồn hơn đầu: {} < {}", peak.valence, start.valence);
        // Cuối hồi phục
        assert!(end.valence > peak.valence,
            "Cuối tốt hơn peak: {} > {}", end.valence, peak.valence);
    }
