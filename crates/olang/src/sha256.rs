...
mod tests {
    ...
    fn test_incremental_update() {
        // Same result whether we update all at once or in pieces
    ...
    #[test]
    fn test_multi_block() {
        // 64+ bytes to test multi-block processing
        let data = [0x61u8; 128]; // 128 'a's
        let mut h = Sha256::new();
        h.update(&data);
        let d = h.finalize();
        // Known: SHA256 of 128 'a' characters
        assert_eq!(
            hex(&d),
