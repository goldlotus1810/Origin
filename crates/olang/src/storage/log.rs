...
pub enum LogEvent {
    ...
    NodePromotedQR { chain_hash: u64, timestamp: i64 },
    /// Silk edge được tạo.
    EdgeCreated {
        from_hash: u64,
        to_hash: u64,
    ...
    /// Dream cycle chạy.
    DreamCycle { proposals: u32, timestamp: i64 },
    /// Lỗi hệ thống.
    SystemError { code: u32, timestamp: i64 },
}
...
mod tests {
    ...
    fn log_append_only() {
        ...
        // Append 5 events
        for i in 0..5u32 {
            log.append(LogEvent::SystemError {
                code: i,
                timestamp: i as i64,
            });
        }
    ...
    fn log_qr_supersede_event() {
        ...
        log.append(LogEvent::QRSuperseded {
            old_hash: 0xDEAD_0000_u64,
            new_hash: 0xBEEF_0000_u64,
            timestamp: 5000,
        });
    ...
    fn log_dream_cycle() {
        let mut log = make_log();
        log.append(LogEvent::DreamCycle {
            proposals: 3,
            timestamp: 10000,
        });
        assert_eq!(log.len(), 1);
        assert!(matches!(
            log.events()[0],
            LogEvent::DreamCycle { proposals: 3, .. }
        ));
    }
}
