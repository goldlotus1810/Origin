...
fn filter_and_write(parsed: &ParsedFile, profile: &DeviceProfile, ts: i64) -> Option<CloneResult> {
    ...
    for node in &parsed.nodes {
        ...
        if writer.size() >= profile.max_bytes {
            break;
        }
    }
    ...
    if writer.append_node(&device_chain, 0, true, ts).is_ok() {
        writer
            .append_alias(&profile.id, device_chain.chain_hash(), ts)
            .ok();
        node_count += 1;
    }
...
pub fn compute_delta(origin_bytes: &[u8], device_bytes: &[u8], ts: i64) -> Option<Vec<u8>> {
    ...
    let origin_hashes: Vec<u64> = origin_parsed
        .nodes
        .iter()
...
mod tests {
    ...
    fn make_origin(ts: i64) -> Vec<u8> {
        ...
        for cp in chains {
            ...
            w.append_alias(&format!("node_{:05X}", cp), chain.chain_hash(), ts)
                .ok();
        }
        w.into_bytes()
    }
    ...
    fn clone_hub_more_than_sensor() {
        ...
        assert!(
            hub_r.node_count >= sensor_r.node_count,
            "Hub giữ ≥ Sensor: {} ≥ {}",
            hub_r.node_count,
            sensor_r.node_count
        );
...
impl WorkerPackage {
    ...
    pub fn from_bytes(b: &[u8]) -> Option<Self> {
        ...
        let created_at =
            i64::from_be_bytes([b[14], b[15], b[16], b[17], b[18], b[19], b[20], b[21]]);
        let olang_len = u32::from_be_bytes([b[22], b[23], b[24], b[25]]) as usize;
        if b.len() < 26 + olang_len {
            return None;
        }
        Some(Self {
            isl_layer,
...
mod package_tests {
    ...
    fn package_magic_check() {
        ...
        assert!(
            WorkerPackage::from_bytes(&bad).is_none(),
            "Bad magic → None"
        );
