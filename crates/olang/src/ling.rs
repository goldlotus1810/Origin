...
static NEGATIONS: &[&str] = &[
    // Vietnamese
    "không", "chẳng", "chưa", "đừng", "chả",
    // English
    "not", "never", "no", "neither", "nor", "without",
    // French
    "ne", "pas", "jamais", "rien", "ni", "sans",
    // German
...
static AMPLIFIERS: &[&str] = &[
    // Vietnamese
    "rất", "quá", "lắm", "cực", "vô cùng", "thực sự", "hết sức",
    "cực kỳ", "siêu", "tuyệt đối",
    // English
    "very", "so", "extremely", "absolutely", "totally", "really",
    "incredibly", "super", "utterly", "deeply",
    // French
    "très", "tellement", "vraiment", "absolument", "extrêmement",
    "trop", "fort", "terriblement",
    // German
    "sehr", "so", "wirklich", "absolut", "total", "unglaublich",
    "extrem", "furchtbar", "schrecklich",
    // Spanish/Portuguese
    "muy", "tan", "realmente", "absolutamente", "totalmente",
    "bastante", "demasiado",
    // Chinese/Japanese
    "非常", "太", "很", "真的", "とても", "すごく",
];
...
static DIMINISHERS: &[&str] = &[
    // Vietnamese
    "hơi", "một chút", "khá", "tương đối", "ít", "đôi chút",
    "nhẹ", "vừa vừa", "cũng được",
    // English
    "rather", "somewhat", "slightly", "a bit", "a little",
    "fairly", "kind of", "sort of", "not very",
    // French
    "assez", "un peu", "plutôt", "légèrement", "relativement",
    "quelque peu",
    // German
    "etwas", "ziemlich", "ein bisschen", "recht", "relativ",
    // Spanish/Portuguese
    "bastante", "algo", "un poco", "relativamente",
    // Chinese/Japanese
    "有点", "稍微", "ちょっと", "少し",
];
...
static CONTRASTS: &[&str] = &[
    // Vietnamese
    "nhưng", "mà", "tuy nhiên", "thế nhưng", "song", "dù", "dẫu",
    "mặc dù", "dù vậy", "tuy vậy",
    // English
    "but", "however", "although", "though", "yet", "still",
    "nevertheless", "nonetheless", "despite",
    // French
    "mais", "cependant", "pourtant", "néanmoins", "quand même",
    "bien que", "malgré",
    // German
    "aber", "jedoch", "obwohl", "trotzdem", "dennoch", "doch",
    "allerdings",
    // Spanish/Portuguese
...
impl LingParser {
    ...
    pub fn tag_word(&self, word: &str) -> PosTag {
        ...
        if self.is_negation(s)   { return PosTag::Modifier(ModifierKind::Negation);   }
        if self.is_amplifier(s)  { return PosTag::Modifier(ModifierKind::Amplifier);  }
        if self.is_diminisher(s) { return PosTag::Modifier(ModifierKind::Diminisher); }
        if self.is_contrast(s)   { return PosTag::Modifier(ModifierKind::Contrast);   }
        if self.is_adjective(s)  { return PosTag::Adjective; }
        if self.is_verb(s)       { return PosTag::Verb; }
        if self.is_noun(s)       { return PosTag::Noun; }
        if self.is_adverb(s)     { return PosTag::Adverb; }
...
impl Default for LingParser { fn default() -> Self { Self::new() } }
...
mod tests {
    ...
    fn parser() -> LingParser { LingParser::new() }

    // ── POS tagging ───────────────────────────────────────────────────────────

    #[test]
    fn tag_negation_vi() {
        assert_eq!(parser().tag_word("không"), PosTag::Modifier(ModifierKind::Negation));
        assert_eq!(parser().tag_word("chẳng"), PosTag::Modifier(ModifierKind::Negation));
        assert_eq!(parser().tag_word("chưa"),  PosTag::Modifier(ModifierKind::Negation));
    }

    #[test]
    fn tag_negation_multilang() {
        assert_eq!(parser().tag_word("not"),   PosTag::Modifier(ModifierKind::Negation));
        assert_eq!(parser().tag_word("pas"),   PosTag::Modifier(ModifierKind::Negation));
        assert_eq!(parser().tag_word("nicht"), PosTag::Modifier(ModifierKind::Negation));
        assert_eq!(parser().tag_word("no"),    PosTag::Modifier(ModifierKind::Negation));
        assert_eq!(parser().tag_word("不"),    PosTag::Modifier(ModifierKind::Negation));
    }

    #[test]
    fn tag_amplifier_vi() {
        assert_eq!(parser().tag_word("rất"),   PosTag::Modifier(ModifierKind::Amplifier));
        assert_eq!(parser().tag_word("quá"),   PosTag::Modifier(ModifierKind::Amplifier));
        assert_eq!(parser().tag_word("lắm"),   PosTag::Modifier(ModifierKind::Amplifier));
        assert_eq!(parser().tag_word("cực"),   PosTag::Modifier(ModifierKind::Amplifier));
    }

    #[test]
    fn tag_amplifier_multilang() {
        assert_eq!(parser().tag_word("very"),  PosTag::Modifier(ModifierKind::Amplifier));
        assert_eq!(parser().tag_word("très"),  PosTag::Modifier(ModifierKind::Amplifier));
        assert_eq!(parser().tag_word("sehr"),  PosTag::Modifier(ModifierKind::Amplifier));
        assert_eq!(parser().tag_word("muy"),   PosTag::Modifier(ModifierKind::Amplifier));
        assert_eq!(parser().tag_word("非常"),  PosTag::Modifier(ModifierKind::Amplifier));
    }

    #[test]
    fn tag_diminisher() {
        assert_eq!(parser().tag_word("hơi"),   PosTag::Modifier(ModifierKind::Diminisher));
        assert_eq!(parser().tag_word("rather"),PosTag::Modifier(ModifierKind::Diminisher));
        assert_eq!(parser().tag_word("etwas"), PosTag::Modifier(ModifierKind::Diminisher));
    }

    #[test]
    fn tag_contrast() {
        assert_eq!(parser().tag_word("nhưng"), PosTag::Modifier(ModifierKind::Contrast));
        assert_eq!(parser().tag_word("but"),   PosTag::Modifier(ModifierKind::Contrast));
        assert_eq!(parser().tag_word("aber"),  PosTag::Modifier(ModifierKind::Contrast));
        assert_eq!(parser().tag_word("mais"),  PosTag::Modifier(ModifierKind::Contrast));
        assert_eq!(parser().tag_word("pero"),  PosTag::Modifier(ModifierKind::Contrast));
    }
    ...
    fn double_negation_positive() {
        let (v, _) = apply_modifiers(
            &["không", "phải", "không", "vui"], 0.8, 0.5,
        );
