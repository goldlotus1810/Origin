...
impl Skill for AnalogySkill {
    ...
    fn execute(&self, ctx: &mut ExecContext) -> SkillResult {
        ...
        let delta_magnitude: f32 = (0..5).map(|i| {
            let d = (bytes_b[i] as f32 - bytes_a[i] as f32).abs();
            d / 255.0
        }).sum::<f32>() / 5.0;

        let confidence = delta_magnitude.clamp(0.1, 0.95);
        ctx.set(String::from("analogy_confidence"), alloc::format!("{:.3}", confidence));
...
impl Skill for AbstractionSkill {
    ...
    fn execute(&self, ctx: &mut ExecContext) -> SkillResult {
        ...
        ctx.set(String::from("abstraction_variance"), alloc::format!("{:.3}", variance));
...
impl Skill for CausalitySkill {
    ...
    fn execute(&self, ctx: &mut ExecContext) -> SkillResult {
        ...
        let co_count: u32 = ctx.get("coactivation_count")
        ...
        if evidence_count < 2 {
            ...
            ctx.set(String::from("causality_verdict"), String::from("insufficient"));
            return SkillResult::Insufficient;
        }
        ...
        ctx.set(String::from("causality_confidence"), alloc::format!("{:.2}", confidence));
...
impl Skill for ContradictionSkill {
    ...
    fn execute(&self, ctx: &mut ExecContext) -> SkillResult {
        ...
        let relation_orthogonal = ma.relation == RelationBase::Orthogonal
            || mb.relation == RelationBase::Orthogonal;
        ...
        if score < 0.3 {
            ctx.set(String::from("contradiction_score"), alloc::format!("{:.2}", score));
            ctx.set(String::from("contradiction_verdict"), String::from("compatible"));
            return SkillResult::Insufficient;
        }
        ...
        ctx.set(String::from("contradiction_score"), alloc::format!("{:.2}", score));
        ctx.set(String::from("contradiction_verdict"), String::from("contradicted"));
...
impl Skill for CuriositySkill {
    ...
    fn execute(&self, ctx: &mut ExecContext) -> SkillResult {
        ...
        let known_count: usize = ctx.get("known_count")
            .and_then(|s| s.parse().ok())
            .unwrap_or(0);
        ...
        let nearest_sim: f32 = ctx.get("nearest_similarity")
            .and_then(|s| s.parse().ok())
            .unwrap_or(0.0);
        ...
        ctx.set(String::from("curiosity_level"), String::from(curiosity_level));
        ctx.set(String::from("curiosity_score"), alloc::format!("{:.3}", novelty));
...
impl Skill for ReflectionSkill {
    ...
    fn execute(&self, ctx: &mut ExecContext) -> SkillResult {
        ...
        let qr_count: usize = ctx.get("qr_count")
            .and_then(|s| s.parse().ok())
            .unwrap_or(0);
        let dn_count: usize = ctx.get("dn_count")
            .and_then(|s| s.parse().ok())
            .unwrap_or(0);
        let edge_count: usize = ctx.get("edge_count")
            .and_then(|s| s.parse().ok())
            .unwrap_or(0);
        ...
        ctx.set(String::from("knowledge_quality"), alloc::format!("{:.3}", quality));
        ctx.set(String::from("proven_ratio"), alloc::format!("{:.3}", proven_ratio));
        ctx.set(String::from("connectivity"), alloc::format!("{:.3}", connectivity));
...
impl Skill for HonestySkill {
    ...
    fn execute(&self, ctx: &mut ExecContext) -> SkillResult {
        ...
        let confidence: f32 = ctx.get("confidence")
            .and_then(|s| s.parse().ok())
            .unwrap_or(0.0);
        ...
        ctx.set(String::from("epistemic_confidence"), alloc::format!("{:.3}", confidence));
...
mod tests {
    ...
    fn chain_fire()  -> MolecularChain { encode_codepoint(0x1F525) } // 🔥
    fn chain_water() -> MolecularChain { encode_codepoint(0x1F4A7) } // 💧
    fn chain_ice()   -> MolecularChain { encode_codepoint(0x2744)  } // ❄
    fn chain_happy() -> MolecularChain { encode_codepoint(0x1F600) } // 😀
    fn chain_angry() -> MolecularChain { encode_codepoint(0x1F621) } // 😡
    fn chain_star()  -> MolecularChain { encode_codepoint(0x2B50)  } // ⭐
    fn chain_heart() -> MolecularChain { encode_codepoint(0x2764)  } // ❤
    fn chain_brain() -> MolecularChain { encode_codepoint(0x1F9E0) } // 🧠
    ...
    fn causality_insufficient_without_evidence() {
        ...
        let r = skill.execute(&mut c);
        assert!(matches!(r, SkillResult::Insufficient), "No evidence → silence");
    }
    ...
    fn contradiction_happy_angry() {
        ...
        assert!(score > 0.0, "Happy vs Angry → contradiction score > 0: {}", score);
    }
    ...
    fn curiosity_known_territory() {
        ...
        let r = skill.execute(&mut c);
        assert!(matches!(r, SkillResult::Insufficient), "Known → no curiosity");
    ...
    fn reflection_strong_knowledge() {
        ...
        assert!(quality > 0.5, "High QR ratio + connected → strong: {}", quality);
    ...
    fn honesty_silence_when_unsure() {
        ...
        let r = skill.execute(&mut c);
        assert!(matches!(r, SkillResult::Insufficient), "Low confidence → silence");
    ...
    fn all_instincts_are_stateless() {
        ...
        for skill in instincts {
            ...
            assert!(matches!(r1, SkillResult::Insufficient),
                "{} phải Insufficient khi không có input", skill.name());
            assert!(matches!(r2, SkillResult::Insufficient),
                "{} stateless — kết quả giống nhau", skill.name());
        }
    }
}
