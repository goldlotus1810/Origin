...
impl Chief {
    ...
    /// Đăng ký Worker mới.
    pub fn register_worker(&mut self, addr: ISLAddress, kind_byte: u8, ts: i64) {
    ...
    pub fn receive_frame(&mut self, frame: ISLFrame, ts: i64) {
        ...
        match frame.header.msg_type {
            ...
            MsgType::Ack => {
                // Worker ack command → OK, không làm gì thêm
            }
            MsgType::Emergency => {
                // Cấp cứu → forward ngay lên AAM
...
mod tests {
    ...
    fn register_worker() {
        ...
        c.register_worker(worker_addr(1), 0x01, 1000);
        c.register_worker(worker_addr(2), 0x01, 1000);
    ...
    fn receive_sensor_creates_report() {
        ...
        c.register_worker(w1, 0x01, 1000);
    ...
    fn heartbeat_marks_dead() {
        let mut c = make_chief();
        c.register_worker(worker_addr(1), 0x01, 0);
