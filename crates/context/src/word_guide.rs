...
pub fn target_affect(curve: &ConversationCurve) -> EmotionTag {
    if curve.turn_count() == 0 {
        return EmotionTag {
            valence: DEFAULT_TARGET_V,
            arousal: DEFAULT_TARGET_A,
            dominance: 0.60,
            intensity: 0.22,
        };
    };
...
pub fn select_words(target: EmotionTag, n: usize) -> Vec<WordCandidate> {
    let mut candidates: Vec<WordCandidate> = CORE_LEXICON
        .iter()
        .map(|e| {
...
impl EmotionHistory {
    ...
    pub fn recent_trend(&self) -> &'static str {
        ...
        let v2: f32 = self.edges[mid..].iter().map(|e| e.tag.valence).sum::<f32>()
            / (self.edges.len() - mid) as f32;
        let diff = v2 - v1;
        if diff > 0.10 {
            "improving"
        } else if diff < -0.10 {
    ...
    pub fn by_source(&self) -> BTreeMap<String, EmotionTag> {
        ...
        buckets
            ...
            .map(|(k, tags)| {
                ...
                let (v, a, d, i) =
                    tags.iter()
                        .fold((0.0f32, 0.0f32, 0.0f32, 0.0f32), |(v, a, d, i), t| {
                            (
                                v + t.valence,
                                a + t.arousal,
                                d + t.dominance,
...
mod tests {
    ...
    fn select_words_nearest_first() {
        ...
        assert!(
            words[0].tag.valence > 0.0,
            "Nearest to V=0.75 phải positive: {} ({})",
            words[0].word,
            words[0].tag.valence
        );
    }
    ...
    fn select_words_sad_target() {
        ...
        assert!(
            words[0].tag.valence < 0.0,
            "Nearest to V=-0.55 phải negative: {}",
            words[0].word
        );
    }
    ...
    fn select_words_scores_ascending() {
        ...
        for i in 1..words.len() {
            assert!(
                words[i].score >= words[i - 1].score,
                "Scores phải tăng dần: {} >= {}",
                words[i].score,
                words[i - 1].score
            );
        }
    ...
    fn affect_components_positive_curve() {
        ...
        let components = affect_components(&curve);
        assert!(
            components.target_v > 0.0,
            "Positive curve → positive target: {}",
            components.target_v
        );
    }
    ...
    fn history_by_source() {
        let mut h = EmotionHistory::new();
        let ctx_real = EmotionContext {
            source: EmotionSource::RealNow,
            ..ctx()
        };
        let ctx_fiction = EmotionContext {
