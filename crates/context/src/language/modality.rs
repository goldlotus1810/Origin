...
impl ImageFeatures {
    ...
    pub fn to_emotion(self) -> EmotionTag {
        ...
        if let Some(fv) = self.face_valence {
            let arousal =
                (self.motion * 0.4 + self.saturation * 0.3 + self.brightness * 0.3).clamp(0.1, 0.9);
            return EmotionTag {
        ...
        let arousal =
            (self.motion * 0.5 + self.saturation * 0.3 + self.brightness * 0.2).clamp(0.1, 0.9);
...
impl BioFeatures {
    ...
    pub fn to_emotion(self) -> EmotionTag {
        ...
        let temp_signal = if self.skin_temp < 31.0 {
            -0.20
        } else if self.skin_temp > 35.0 {
            0.10
        } else {
            0.0
        };
...
pub fn fuse_all(
    ...
    if let Some(a) = audio {
        inputs.push(a.to_modality_input(0.85));
    }
    if let Some(i) = image {
        inputs.push(i.to_modality_input(0.70));
    }
    if let Some(b) = bio {
        inputs.push(b.to_modality_input(0.95));
    }
...
mod tests {
    ...
    fn voice_break_distress() {
        ...
        let breaking = AudioFeatures {
            voice_break: 0.8,
            ..normal
        };
        assert!(
            breaking.to_emotion().valence < normal.to_emotion().valence,
    ...
    fn fast_tempo_high_arousal() {
        ...
        assert!(
            fast.to_emotion().arousal > slow.to_emotion().arousal,
    ...
    fn blue_hue_negative() {
        ...
        assert!(
            img.to_emotion().valence < 0.1,
            "Xanh dương + tối → neutral/negative"
        );
    }
    ...
    fn face_detection_overrides_color() {
        ...
        assert!(
            img.to_emotion().valence > 0.5,
            "Face override màu sắc: {}",
            img.to_emotion().valence
        );
    }

    #[test]
    fn motion_increases_arousal() {
        let still = ImageFeatures {
            motion: 0.0,
            hue: 120.0,
            saturation: 0.5,
            brightness: 0.6,
        ...
        let moving = ImageFeatures {
            motion: 0.9,
            ..still
        };
        assert!(
            moving.to_emotion().arousal > still.to_emotion().arousal,
