...
pub fn sentence_affect(
    ...
    let mut composite = word_emotions.first()
        .copied()
    ...
    for i in 1..word_hashes.len().min(word_emotions.len()) {
        ...
        let edge_weight = graph.assoc_weight(word_hashes[i-1], hash)
...
pub fn response_tone(curve: &[f32]) -> ResponseTone {
    let n = curve.len();
    if n == 0 { return ResponseTone::Engaged; }

    let v = curve[n - 1];

    // f'(t) — tốc độ thay đổi
    let d1 = if n >= 2 { curve[n-1] - curve[n-2] } else { 0.0 };
    ...
    // Ưu tiên theo thứ tự
    if d1 < -0.15                      { ResponseTone::Supportive }
    else if d2 < -0.25                 { ResponseTone::Pause }
    else if d1 > 0.15                  { ResponseTone::Reinforcing }
    else if d2 > 0.25 && v > 0.0      { ResponseTone::Celebratory }
    else if v < -0.20                  { ResponseTone::Gentle }
    else                               { ResponseTone::Engaged }
}
...
mod tests {
    ...
    fn sentence_affect_amplifies_connected() {
        ...
        assert!(result.composite.valence < simple_avg,
            "SentenceAffect < average: {} < {}", result.composite.valence, simple_avg);
    }
    ...
    fn sentence_affect_unconnected_mild() {
        ...
        assert!((result.composite.valence - avg).abs() < 0.3,
            "Không Silk → gần avg: {} ≈ {}", result.composite.valence, avg);
    }
    ...
    fn sentence_affect_depth_limit() {
        ...
        assert!(result.path.len() <= 5, "Depth limit: {} <= 5", result.path.len());
    }
    ...
    fn sentence_affect_valence_clamped() {
        ...
        let result = sentence_affect(
            &g,
            &[0xA],
            &[EmotionTag::new(-2.0, 2.0, 1.5, 1.5)],
            10,
        );
    ...
    fn tone_supportive_falling() {
        ...
        assert_eq!(response_tone(&curve), ResponseTone::Supportive,
    ...
    fn tone_pause_sudden_drop() {
        ...
        assert!(matches!(tone, ResponseTone::Supportive | ResponseTone::Pause));
    }

    #[test]
    fn tone_reinforcing_rising() {
        let curve = [-0.5, -0.2, 0.1];
        assert_eq!(response_tone(&curve), ResponseTone::Reinforcing,
    ...
    fn tone_celebratory() {
        ...
        assert!(matches!(tone3, ResponseTone::Celebratory | ResponseTone::Engaged));
    }

    #[test]
    fn tone_gentle_stable_sad() {
        let curve = [-0.4, -0.4, -0.4]; // Buồn ổn định
        assert_eq!(response_tone(&curve), ResponseTone::Gentle,
    ...
    fn step_large_delta_capped() {
        ...
        assert!((result - (-0.30)).abs() < 0.001,
            "Bước tối đa 0.40: {} ≈ -0.30", result);
    }
    ...
    fn step_no_jump_more_than_040() {
        ...
        assert!((step - start).abs() <= 0.40 + 0.001,
            "Không nhảy quá 0.40: delta={}", (step-start).abs());
    }
}
