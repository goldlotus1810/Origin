...
fn shape_of(cp: u32, name: &str) -> u8 {
    ...
    if (0x2580..=0x259F).contains(&cp) { return 0x03; } // ░▒▓█ → Box
    // Arrows 2190..21FF → Cone (directional)
    if (0x2190..=0x21FF).contains(&cp) { return 0x04; }
    // Math Operators 2200..22FF
    if (0x2200..=0x22FF).contains(&cp) {
        // Set operations → Union/Intersect/Subtract
        if name.contains("UNION")        { return 0x06; }
        if name.contains("INTERSECTION") { return 0x07; }
        if name.contains("MINUS")        { return 0x08; }
        return 0x05; // Torus — vòng lặp toán học
    }
    // Letterlike 2100..214F → Torus (abstract loops)
    if (0x2100..=0x214F).contains(&cp) { return 0x05; }
    // EMOTICON → Sphere (entities are round)
    if (0x2600..=0x26FF).contains(&cp)   { return 0x01; }
    if (0x1F300..=0x1FAFF).contains(&cp) { return 0x01; }
    // Musical → Torus (cycles)
    if (0x1D100..=0x1D1FF).contains(&cp) { return 0x05; }
    if (0x4DC0..=0x4DFF).contains(&cp)   { return 0x05; }
    0x01 // default: Sphere
}
...
fn relation_of(cp: u32, name: &str) -> u8 {
    ...
    // Math Operators 2200..22FF
    if (0x2200..=0x22FF).contains(&cp) {
        if name.contains("ELEMENT")   { return 0x01; } // ∈ Member
        if name.contains("SUBSET")    { return 0x02; } // ⊂ Subset
        if name.contains("IDENTICAL") { return 0x03; } // ≡ Equiv
        if name.contains("EQUAL")     { return 0x03; }
        if name.contains("TACK")      { return 0x04; } // ⊥ Orthogonal
        if name.contains("RING")      { return 0x05; } // ∘ Compose
        if name.contains("ARROW")     { return 0x06; } // → Causes
        if name.contains("ALMOST")    { return 0x07; } // ≈ Similar
        if name.contains("UNION")     { return 0x01; } // ∪ Member (contains)
        return 0x03; // Equiv default for math
    }
    // Arrows 2190..21FF
    if (0x2190..=0x21FF).contains(&cp) {
        if name.contains("LEFT")  { return 0x08; } // ← DerivedFrom
        if name.contains("RIGHT") { return 0x06; } // → Causes
        if name.contains("UP")    { return 0x06; } // ↑ Causes (upward)
        if name.contains("DOWN")  { return 0x08; } // ↓ DerivedFrom (downward)
        return 0x06; // Causes default
    }
    // Block Elements → Member (fill = belonging to level)
    if (0x2580..=0x259F).contains(&cp) { return 0x01; }
    // EMOTICON → Member (entities belong to groups)
    if (0x2600..=0x26FF).contains(&cp)   { return 0x01; }
    if (0x1F300..=0x1FAFF).contains(&cp) { return 0x01; }
    // Musical → Similar (notes relate to each other)
    if (0x1D100..=0x1D1FF).contains(&cp) { return 0x07; }
    0x01 // default: Member
}
...
fn valence_of(cp: u32, name: &str) -> u8 {
    ...
    if (0x1F600..=0x1F64F).contains(&cp) {
        ...
        if (0x1F600..=0x1F60F).contains(&cp) { return 0xFF; }
        // Negative faces 1F620..1F62F
        if (0x1F620..=0x1F62F).contains(&cp) { return 0x00; }
        // Neutral 1F610..1F61F
        if (0x1F610..=0x1F61F).contains(&cp) { return 0x80; }
    }
    // Symbols by name
    if name.contains("FIRE") || name.contains("FLAME")   { return 0xFF; }
    if name.contains("HEART") || name.contains("LOVE")   { return 0xFF; }
    if name.contains("STAR") || name.contains("SPARKL")  { return 0xE0; }
    if name.contains("SUN") || name.contains("BRIGHT")   { return 0xE0; }
    if name.contains("WARNING") || name.contains("DANGE") { return 0x20; }
    if name.contains("SKULL") || name.contains("DEATH")  { return 0x00; }
    if name.contains("DROPLET") || name.contains("WATER") { return 0xC0; }
    if name.contains("SNOWFLAKE") || name.contains("COLD") { return 0x30; }
    if name.contains("BRAIN")    { return 0xC0; }
    if name.contains("CHECK")    { return 0xFF; }
    if name.contains("CROSS")    { return 0x10; }
    // Math → neutral
    if (0x2200..=0x22FF).contains(&cp) { return 0x80; }
    if (0x2100..=0x214F).contains(&cp) { return 0x80; }
    // Musical → slightly positive
    if (0x1D100..=0x1D1FF).contains(&cp) { return 0xA0; }
...
fn arousal_of(cp: u32, name: &str) -> u8 {
    ...
    if (0x1D100..=0x1D1FF).contains(&cp) {
        if name.contains("FORTISSIMO") || name.contains("FF") { return 0xFF; }
        if name.contains("FORTE")                             { return 0xC0; }
        if name.contains("MEZZO")                             { return 0x80; }
        if name.contains("PIANO") && name.contains("PIANO")  { return 0x10; }
        if name.contains("PIANO")                             { return 0x40; }
        return 0x80;
    }
    // EMOTICON energy
    if (0x1F600..=0x1F64F).contains(&cp) {
        if (0x1F600..=0x1F60F).contains(&cp) { return 0xFF; } // excited positive
        if (0x1F620..=0x1F62F).contains(&cp) { return 0xE0; } // excited negative
        if (0x1F610..=0x1F61F).contains(&cp) { return 0x20; } // calm neutral
    }
    // By name
    if name.contains("FIRE") || name.contains("LIGHTNING") { return 0xFF; }
    if name.contains("WARNING") || name.contains("ALARM")  { return 0xE0; }
    if name.contains("SNOWFLAKE") || name.contains("SLEEP") { return 0x10; }
    if name.contains("DROPLET")   { return 0x40; }
    if name.contains("BRAIN")     { return 0x80; }
    if name.contains("RUNNER")    { return 0xD0; }
    if name.contains("STOP")      { return 0x20; }
    // Math → static (arousal near 0)
    if (0x2200..=0x22FF).contains(&cp) { return 0x20; }
    if (0x2100..=0x214F).contains(&cp) { return 0x10; }
    // Arrows → moderate (directed action)
    if (0x2190..=0x21FF).contains(&cp) { return 0xC0; }
...
fn time_of(cp: u32, name: &str) -> u8 {
    ...
    if (0x1D100..=0x1D1FF).contains(&cp) {
        if name.contains("WHOLE")     { return 0x01; } // Static (Largo)
        if name.contains("HALF")      { return 0x02; } // Slow (Adagio)
        if name.contains("QUARTER")   { return 0x03; } // Medium (Andante)
        if name.contains("EIGHTH")    { return 0x04; } // Fast (Allegro)
        if name.contains("SIXTEENTH") { return 0x05; } // Instant (Presto)
        return 0x03; // Medium default
    }
    // Math symbols = static (mathematical truths không thay đổi)
    if (0x2200..=0x22FF).contains(&cp) { return 0x01; } // Static
    if (0x2100..=0x214F).contains(&cp) { return 0x01; } // Static
    if (0x2150..=0x218F).contains(&cp) { return 0x01; } // Static (numbers)
    // Geometric shapes = static
    if (0x25A0..=0x25FF).contains(&cp) { return 0x01; } // Static
    // Block elements = static
    if (0x2580..=0x259F).contains(&cp) { return 0x01; } // Static
    // Arrows = instant (directed, immediate)
    if (0x2190..=0x21FF).contains(&cp) { return 0x05; } // Instant
    // EMOTICON by name
    if name.contains("FIRE") || name.contains("LIGHTNING") { return 0x04; } // Fast
    if name.contains("SNOWFLAKE") || name.contains("MOON")  { return 0x02; } // Slow
    if name.contains("DROPLET")   { return 0x02; } // Slow
    if name.contains("RUNNER")    { return 0x04; } // Fast
    if name.contains("STOP")      { return 0x05; } // Instant
    if name.contains("BRAIN")     { return 0x03; } // Medium
    if name.contains("CLOCK")     { return 0x05; } // Instant
    // General EMOTICON = medium (thực thể có vận động bình thường)
    if (0x2600..=0x26FF).contains(&cp)   { return 0x03; }
    if (0x1F300..=0x1FAFF).contains(&cp) { return 0x03; }
    // Yijing = slow cycle
    if (0x4DC0..=0x4DFF).contains(&cp) { return 0x02; }
    0x03 // default Medium
}
...
fn main() {
    ...
    for line in content.lines() {
        ...
        let name = parts[1];
        let cat  = parts[2];
        if name.starts_with('<') { continue; } // skip range markers
    ...
    for group in GROUPS {
        for &(start, end) in group.ranges {
            for cp in start..=end {
                if seen_cp.contains_key(&cp) { continue; }
                ...
                entries.push(Entry {
                    cp, group: group.byte,
                    shape, relation, valence, arousal, time, hash,
                    name: name.clone(),
                });
            }
        }
    ...
    let mut hash_to_cp: Vec<(u64, u32)> = entries.iter()
        .map(|e| (e.hash, e.cp))
        .collect();
    ...
    for e in &entries {
        buckets.entry((e.shape, e.relation))
               .or_default()
               .push(e.cp);
    }
    ...
    writeln!(src, "    pub hash:     u64,  // FNV-1a của [shape,rel,val,aro,time]").unwrap();
    ...
    eprintln!("cargo:warning=Generated: {} entries, {} hash entries, {} buckets",
        entries.len(), hash_to_cp.len(), bucket_list.len());
}
